...
The v0.2 source-transfer path remains temporarily available only to phases that have not yet migrated; ADRs 0032, 0033, and 0034 have since moved implementation, Checks, and Reviews. It is not a fallback for a failed shared-Workspace launch. A real static-image Session proof remains opt-in and version-bound to the configured OpenShell gateway, Docker driver, image, and policy.
